...
public class AcuteConditionRule implements ProcessingRule {
    ...
    public AcuteConditionRule(String rhRegisterId, ImmutableSet<String> rhFactorParas, String bopRegisterId, ImmutableSet<String> bopFactorParas) {
    ...
    public Optional<SoP> getApplicableSop(Condition condition, ServiceHistory serviceHistory, Predicate<Deployment> isOperational, CaseTrace caseTrace) {
        ...
        {
            caseTrace.addReasoningFor(ReasoningFor.ABORT_PROCESSING,String.format("The service history does not show any continuous full time service on the condition onset date (%s).  This is required for the condition '%s'.",condition.getStartDate(),condition.getSopPair().getConditionName()));
        ...
        Function<ServiceHistory, Optional<SoP>> determineApplicableSop = sh -> {
            ...
            if (!deploymentDuringWhichConditionOccurred.isPresent()) {
                caseTrace.addReasoningFor(ReasoningFor.STANDARD_OF_PROOF,"The standard of proof is balance of probabilities because the service history shows no deployment on the condition start date of " + DateTimeFormatter.BASIC_ISO_DATE.format(condition.getStartDate()));
    ...
    public Recommendation inferRecommendation(ImmutableList<FactorWithSatisfaction> factors, ServiceHistory serviceHistory, SoP applicableSop, Condition condition, Predicate<Deployment> isOperational, CaseTrace caseTrace)
    {
        ...
        boolean hasOperationalServiceBeforeOnset = serviceHistory.getNumberOfDaysOfFullTimeOperationalService(serviceHistory.getStartofService().get(), condition.getStartDate(),isOperational) > 0;
        ...
        if (satisfied && standardOfProof == StandardOfProof.BalanceOfProbabilities && hasOperationalServiceBeforeOnset)
        {
        ...
        if (satisfied && !hasOperationalServiceBeforeOnset && standardOfProof == StandardOfProof.BalanceOfProbabilities)
        {
        ...
        if (!satisfied && hasOperationalServiceBeforeOnset) {
